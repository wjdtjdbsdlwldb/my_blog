<?php
include "../part/head.php";
?>

<div class="coding-top-bar con">
    <h1 class="coding-title">WEB CODING</h1>
    <div class="sub-icon"><img src="/resource/img/sub-icon.png" alt=""></div>
    <div class="coding-under-line"></div>
</div>

<ul class="coding con row">
   
    <li class="cell img-box">
        <a href="#" style="background-image:url(https://wjdtjdbsdlwldb.github.io/img1/blog/article/2020/web-coding.jpg);"></a>
        <div>
            <h3>futurasoft</h3>
            <h5>web-coding</h5>
         </div>
         <a href="futurasoft.html" target="_blank" class="link-bl"></a>
    </li>

    <li class="cell img-box">
        <a href="#" style="background-image:url(https://wjdtjdbsdlwldb.github.io/img1/blog/article/2020/gm1.png);"></a>
        <div>
            <h3>GENTLE MONSTER</h3>
            <h5>web-coding</h5>
         </div>
         <a href="./pf/gm/index.php" target="_blank" class="link-bl"></a>
    </li>

    <li class="cell img-box">
        <a href="#" style="background-image:url(https://raw.githubusercontent.com/wjdtjdbsdlwldb/img1/master/blog/article/2020/su.png);"></a>
        <div>
            <h3>SUBWAY</h3>
            <h5>web-coding</h5>
         </div>
         <a href="./pf/subway/index.php" target="_blank" class="link-bl"></a>
    </li>

    <li class="cell img-box">
        <a href="#" style="background-image:url(https://raw.githubusercontent.com/wjdtjdbsdlwldb/img1/master/blog/article/2020/md.jpg);"></a>
        <div>
            <h3>MARKET DESIGNERS</h3>
            <h5>web-coding</h5>
         </div>
         <a href="./pf/md/index.php" target="_blank" class="link-bl"></a>
    </li>
</ul>




<?php
include "../part/foot.php";
?>
